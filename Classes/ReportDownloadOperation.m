	...
	@autoreleasepool {
		...
		for (NSString *dateType in [NSArray arrayWithObjects:@"Daily", @"Weekly", nil]) {
			...
			for (NSString *reportDateString in availableReportDateStrings) {
				...
				NSString *escapedUsername = (__bridge_transfer NSString *)CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (__bridge CFStringRef)username, NULL, CFSTR("!*'();:@&=+$,/?%#[]"), kCFStringEncodingUTF8);
				NSString *escapedPassword = (__bridge_transfer NSString *)CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (__bridge CFStringRef)password, NULL, CFSTR("!*'();:@&=+$,/?%#[]"), kCFStringEncodingUTF8);
