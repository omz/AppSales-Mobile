...
- (NSArray*)translateMultipleText:(NSArray*)sourceArray fromLanguage:(NSString*)sourceLanguage toLanguage:(NSString*)targetLanguage {
    ...
    id contentsJSON = [contents objectFromJSONString];
    [contents release];
    ...
    //    "responseStatus": 200}
    const NSString *responseStatusKey = @"responseStatus";
    const NSString *responseDataKey = @"responseData";
    const NSString *translatedTextKey = @"translatedText";
    const NSString *responseDetailsKey = @"responseDetails";
        
    NSInteger responseStatus = [[contentsJSON objectForKey:responseStatusKey] integerValue];
    id responseData = [contentsJSON objectForKey:responseDataKey];
        ...
        case 200: // translated ok
            if ([responseData isKindOfClass:[NSArray class]]) {
                ...
                for (NSUInteger i=0; i < [responseData count]; i++) {
                    ...
                    NSInteger subResponseStatus = [[element objectForKey:responseStatusKey] integerValue];
                    id subResponseData = [element objectForKey:responseDataKey];
                    if (subResponseStatus == 200) {
                        NSString *translated = [[subResponseData objectForKey:translatedTextKey] removeHtmlEscaping];
                        [results addObject:translated];
                    } else {
                        NSString *original = [sourceArray objectAtIndex:i];
                        [results addObject:original];
                        NSLog(@"could not translate - %@ - %@", [element objectForKey:responseDetailsKey], original);
                    }
                }
                return results;
            } // else, single string translation
            NSString *translated = [[responseData objectForKey:translatedTextKey] removeHtmlEscaping];
