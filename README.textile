...
* app review fetching is done in parallel (and now an order of magnitude faster than before)
* importing of CSV files:
  Copy any old CSV report files into the 'Prefetched' folder, and they'll be automatically imported the first time launching the app (which may take a few seconds to process everything)
